<div align="center">

<img src="https://capsule-render.vercel.app/api?type=waving&color=0:0F2027,50:203A43,100:2C5364&height=220&section=header&text=Harshit%20Mittal&fontSize=60&fontColor=FFFFFF&animation=fadeIn&fontAlignY=38&desc=Full%20Stack%20%2B%20DevOps%20Engineer%20%7C%20Competitive%20Programmer&descAlignY=58&descSize=18" width="100%"/>

<a href="https://harshitt-mittal-portfolio.vercel.app/" target="_blank">Portfolio</a> ·
<a href="https://www.linkedin.com/in/harshit-mittal-1a7707245/" target="_blank">LinkedIn</a> ·
<a href="mailto:hmittal0418@gmail.com">Email</a> ·
<a href="https://leetcode.com/u/harshit_0418/" target="_blank">LeetCode</a>

<img src="https://komarev.com/ghpvc/?username=harshitt0418&label=Profile%20Views&color=2C5364&style=flat" alt="profile views"/>

</div>

<br/>

## `whoami`

```yaml
name: Harshit Mittal
role: Full-Stack Developer · DevOps Enthusiast
degree: B.Tech CSE (Full Stack & DevOps Specialisation), DIT University
cgpa: 8.44 / 10.0
location: Dehradun, Uttarakhand, India
languages: [C++, JavaScript, Python]
currently_building: real-time & security-hardened MERN products
currently_solving: 600+ DSA problems across LeetCode , Codeforces & GFG
fun_fact: my apps get pen-tested by me before anyone else gets the chance
```

<br/>

## Competitive Edge

<div align="center">

| Platform | Rating / Standing | Highlight |
|---|---|---|
| 🟧 **LeetCode** | `1,916+` — Knight, Top 4.16% (875K+ users) | Rank **445 / 39,000+** in Biweekly Contest 186 · Rank **538 / 39,900+** in Biweekly Contest 187 |
| 🟫 **CodeChef** | Top 2.53% | Rank **456 / 18,000+** in Starters 213 |
| 🧮 **DSA** | 600+ problems solved | Arrays · Trees · Graphs · Dynamic Programming |
| 🏆 **CodeStorm 2025** | 🥉 3rd Place | Built *ShareMyThaali* — CodeGenX DITU × InterviewBuddy & Unstop |

</div>

<br/>

## Tech Stack

<div align="center">

**Frontend**
<br/>
<img src="https://skillicons.dev/icons?i=react,nextjs,html,css,tailwind&theme=dark"/>

**Backend & APIs**
<br/>
<img src="https://skillicons.dev/icons?i=nodejs,express&theme=dark"/>
<img src="https://img.shields.io/badge/REST_APIs-000000?style=for-the-badge&logo=fastapi&logoColor=white"/>
<img src="https://img.shields.io/badge/JWT_Auth-000000?style=for-the-badge&logo=jsonwebtokens&logoColor=white"/>

**Databases**
<br/>
<img src="https://skillicons.dev/icons?i=mongodb,postgres,mysql,redis&theme=dark"/>

**Real-Time & Comms**
<br/>
<img src="https://img.shields.io/badge/Socket.io-010101?style=for-the-badge&logo=socketdotio&logoColor=white"/>
<img src="https://img.shields.io/badge/WebSockets-2C5364?style=for-the-badge&logo=websocket&logoColor=white"/>
<img src="https://img.shields.io/badge/WebRTC-333333?style=for-the-badge&logo=webrtc&logoColor=white"/>

**DevOps & Cloud**
<br/>
<img src="https://skillicons.dev/icons?i=git,github,docker,aws,cloudinary&theme=dark"/>
<img src="https://img.shields.io/badge/CI/CD-2088FF?style=for-the-badge&logo=githubactions&logoColor=white"/>

**Languages**
<br/>
<img src="https://skillicons.dev/icons?i=cpp,js,py&theme=dark"/>

</div>

<br/>

## Featured Builds

<table width="100%">
<tr>
<td width="33%" valign="top">

### 🏠 RentSpace
**P2P Community Rental Marketplace**

Production MERN app serving **30+ real users** — real-time chat via Socket.io, JWT rotation + Google OAuth 2.0, Redis caching, Cloudinary storage. Deployed on Vercel/Render with CORS hardening & rate-limiting.

`MongoDB` `Express` `React` `Node.js` `Socket.io` `Redis`

</td>
<td width="33%" valign="top">

### 🖊️ NexusBoard
**Real-Time Collaborative Whiteboard**

MERN + PostgreSQL whiteboard with freehand drawing, undo/redo, live cursor sync, WebRTC video/audio, and typing-indicator chat. CI/CD pipeline automates AWS-compatible deployments.

`PostgreSQL` `Socket.io` `WebRTC` `AWS` `CI/CD`

</td>
<td width="33%" valign="top">

### 🎙️ Jarvis Browser Copilot
**AI-Powered Chrome Extension**

Manifest V3 extension with **200+ voice commands** (Web Speech API), dual-AI panel (Groq Llama 3.3 70B + Gemini 2.0 Flash) for summarisation & multi-tab research, sub-200ms load time.

`Chrome APIs` `Groq` `Gemini` `OOP`

</td>
</tr>
</table>

<br/>

## What I Actually Do When I Build

```
security first     → JWT rotation, OAuth 2.0, Helmet headers, rate-limiting — every project, no exceptions
real-time by default → Socket.io / WebRTC wherever collaboration or live sync adds value
ship, then scale    → Redis caching + Cloudinary + CI/CD once real users show up
break my own stuff  → found & patched 3-4 critical auth vulnerabilities within hours across projects
```

<br/>

## GitHub Analytics

<div align="center">

<img height="165em" src="https://github-readme-stats.vercel.app/api?username=harshitt0418&show_icons=true&theme=tokyonight&hide_border=true&bg_color=0D1117&title_color=58A6FF&icon_color=58A6FF&text_color=C9D1D9"/>
<img height="165em" src="https://github-readme-stats.vercel.app/api/top-langs/?username=harshitt0418&layout=compact&theme=tokyonight&hide_border=true&bg_color=0D1117&title_color=58A6FF&text_color=C9D1D9"/>

<img src="https://github-readme-streak-stats.herokuapp.com/?user=harshitt0418&theme=tokyonight&hide_border=true&background=0D1117&ring=58A6FF&fire=58A6FF&currStreakLabel=58A6FF"/>

<img src="https://github-readme-activity-graph.vercel.app/graph?username=harshitt0418&theme=tokyo-night&hide_border=true&bg_color=0D1117&color=58A6FF&line=58A6FF&point=C9D1D9"/>

</div>

<br/>

## Let's Connect

<div align="center">

<a href="https://www.linkedin.com/in/harshit-mittal-1a7707245/" target="_blank"><img src="https://img.shields.io/badge/LinkedIn-0A66C2?style=for-the-badge&logo=linkedin&logoColor=white"/></a>
<a href="mailto:hmittal0418@gmail.com"><img src="https://img.shields.io/badge/Gmail-D14836?style=for-the-badge&logo=gmail&logoColor=white"/></a>
<a href="https://harshitt-mittal-portfolio.vercel.app/" target="_blank"><img src="https://img.shields.io/badge/Portfolio-2C5364?style=for-the-badge&logo=vercel&logoColor=white"/></a>
<a href="https://leetcode.com/u/harshit_0418/" target="_blank"><img src="https://img.shields.io/badge/LeetCode-FFA116?style=for-the-badge&logo=leetcode&logoColor=black"/></a>

<br/><br/>

📍 Dehradun, Uttarakhand, India &nbsp;|&nbsp; 📱 +91-7460028542

<br/>

<img src="https://capsule-render.vercel.app/api?type=waving&color=0:2C5364,50:203A43,100:0F2027&height=100&section=footer" width="100%"/>

</div>
